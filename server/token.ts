...
export interface TokenStatus extends TokenClaims {
  autoRefresh: boolean;
...
export class TokenManager {
  ...
  private lastRefresh: RefreshOutcome | null = null;

  constructor(baseDir: string, serial: string) {
  ...
  claims(): TokenClaims {
    if (!this.token) return { issuedAt: null, expiresAt: null };
    try {
  ...
  private readCredentials(): Credentials | null {
    ...
    try {
      const parsed = JSON.parse(fs.readFileSync(this.credentialsFile, 'utf8')) as Partial<Credentials>;
  ...
  get autoRefreshConfigured(): boolean {
  ...
  isDue(now = Date.now() / 1000): boolean {
    const { issuedAt, expiresAt } = this.claims();
    if (!this.token) return true;
    if (issuedAt) return now - issuedAt > REFRESH_AFTER_DAYS * DAY_SECONDS;
    // No readable iat — refresh when less than 11 months of validity remain.
    if (expiresAt) return expiresAt - now < 335 * DAY_SECONDS;
    return false;
  }
   ...
   */
  async refresh(force = false): Promise<RefreshOutcome | null> {
    const credentials = this.readCredentials();
    if (!credentials) return null;
    if (!force && !this.isDue()) return null;

    const record = (ok: boolean, message: string): RefreshOutcome => {
    ...
    try {
      ...
      const session = (await login.json()) as { session_id?: string };
      if (!session.session_id) return record(false, 'Enlighten login returned no session');
      ...
      if (!minted.ok) return record(false, `token endpoint returned HTTP ${minted.status}`);

      const fresh = (await minted.text()).trim();
      if (!fresh.includes('.')) return record(false, 'token endpoint returned something that is not a JWT');
